...
export function Section() {
  ...
  return (
    <div className="h-[calc(100vh-80px)] w-full bg-[#171717] p-2 xl:h-[calc(100vh-112px)] xl:p-8">
      <div className="relative flex h-full w-full flex-col items-center justify-between gap-2 overflow-hidden rounded-lg border border-[#99D420] bg-[url('/calculator-bg.png')] bg-cover bg-center bg-no-repeat p-2 lg:flex-row lg:gap-4 lg:p-4 xl:gap-20 xl:p-20 xl:p-8">
        <div className="absolute left-0 top-0 z-50 h-full w-full bg-black/80" />
        <div className="z-[60] h-2/5 w-full rounded-lg lg:h-full lg:w-[400px] xl:w-[600px] 3xl:w-[800px]">
          <iframe
            width="100%"
            height="100%"
            className="h-full w-full rounded-lg border border-[#99D420]"
            src="https://www.youtube.com/embed/kKWARIHuJkY"
            title="YouTube video player"
            referrerPolicy="strict-origin-when-cross-origin"
            allowFullScreen
          />
        </div>
        <div className="z-[60] flex h-full w-full flex-1 flex-col justify-end rounded-lg border border-[#99D420] p-2 xl:p-8">
          <div className="relative flex h-full w-full flex-col">
            <div
              className={cn(
                "absolute left-1/2 z-[70] flex -translate-x-1/2 flex-col items-center justify-center gap-4 transition duration-1000",
                isClicked
                  ? "-translate-y-5 2xl:-translate-y-[30%]"
                  : "translate-y-1/2",
              )}
            >
              <Image
              ...
              <div
                className={cn(
                  "flex flex-col items-center gap-2 text-center transition duration-500",
                  isClicked ? "opacity-0" : "opacity-100",
                )}
              >
                <span className="text-center text-base font-bold text-white xl:text-2xl 2xl:text-4xl">
                  Faça o
                  <span className="text-[#99D420]">{""} Diagnóstico Power</span>
                </span>
                <span className="text-sm text-white xl:text-lg 2xl:text-3xl">
                  e descubra <span className="text-[#99D420]">a verdade</span>
                  {""} sobre sua Empresa
                </span>
              </div>
            </div>
            ...
            >
              <ScrollArea className="h-full w-full pb-2 pt-5 xl:pt-0">
                {messages.map((message, index) => (
                  ...
                  >
                    {message.role === "user" ? (
                      <div className="flex max-w-[80%] justify-end gap-2 p-1 text-end">
                        <div className="flex flex-col text-white">
                          <span className="mb-2 ml-auto w-max text-[10px] font-semibold xl:text-lg">
                            Eu
                          </span>
                          <div className="rounded-2xl rounded-tr-none border border-white p-4">
                      ...
                      <div className="flex max-w-[80%] justify-start gap-2 p-1 text-start">
                        <Image
                          src="/icon-white.svg"
                          alt=""
                          width={250}
          ...
          <div className="flex w-full flex-row items-center gap-1">
            <div className="flex flex-row items-center gap-1">
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button className="relative flex h-6 w-6 items-center justify-center overflow-hidden rounded-lg border border-zinc-500 p-0.5 xl:h-11 xl:w-11">
                      <Image
                        src={"/pdf3.svg"}
                        alt=""
                        width={100}
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button className="relative flex h-6 w-6 items-center justify-center rounded-lg border border-zinc-500 p-0.5 xl:h-11 xl:w-11">
                      <Image
                        src={"/photo3.svg"}
                        alt=""
                        width={100}
            ...
            <label
              ...
              className="flex h-6 flex-1 items-center overflow-hidden rounded-lg border border-zinc-500 pl-2 xl:h-11 xl:pl-4 xl:pr-2"
            >
              {fileData && fileData.mimeType.startsWith("audio/") ? (
                <>
                  ...
                  <button onClick={() => clearFileData()}>
                    <X className="h-4 text-red-500 xl:h-8" />
                  </button>
                </>
              ) : (
                <div className="flex flex-1 flex-row items-center gap-1">
                  <input
                    className="flex-1 border-none bg-transparent text-[10px] text-white outline-none placeholder:text-zinc-500 focus:outline-none xl:text-base"
              ...
              >
                ...
                {fileData?.mimeType.startsWith("audio/") ? (
                  <Send className="h-4 text-zinc-500 xl:h-8" />
                ) : isRecording ? (
                  <Square className="h-4 text-zinc-500 xl:h-8" />
                ) : (
                  <Mic className="h-4 text-zinc-500 xl:h-8" />
                )}
              </button>
            </label>
          </div>
